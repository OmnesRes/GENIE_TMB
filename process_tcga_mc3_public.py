...
read_path = 'ATGC/files/'
write_path = 'your path to data'
...
tcga_sample_table = pd.read_csv(read_path + 'TCGA-CDR-SupplementalTableS1.tsv', sep='\t').iloc[:, 1:]
tcga_sample_table['histological_type'].fillna('', inplace=True)

# tumor type table to be process by human (asb) review to make the pathology column
# tumor_types = tcga_sample_table.groupby(['type', 'histological_type']).size().to_frame().reset_index()
# tumor_types.to_csv('tumor_types.tsv', sep='\t')

# load pathology annotations from asb
tumor_types = pd.read_csv(read_path + 'tumor_types_NCI-T.csv', sep='\t')
...
mc3_file_name = read_path + 'mc3.v0.2.8.PUBLIC.maf'
...
tcga_counts['non_syn_tmb'] = tcga_counts['non_syn_counts'] / 33.4
...
mantis_file_name = read_path + 'NIHMS962713-supplement-File_S2.csv'
...
#add msi pcr labels, data processing and data sources available at /home/janaya2/Desktop/controlled_maf/msi_ground_truth/
msi = pickle.load(open(read_path + 'msi_ground_truth/msi_labels.pkl', 'rb'))
...
# TCR reads file from pancan
tcr_file = read_path + 'TCGA_mitcr_cdr3_result_161008.tsv'
# columns to load
usecols = ['AliquotBarcode', 'SampleBarcode', 'ParticipantBarcode', 'chain', 'aaSeq', 'vSegment', 'dSegment', 'jSegment', 'abundance']
tcga_tcr = pd.read_csv(tcr_file, sep='\t', usecols=usecols, low_memory=False).rename(columns={'SampleBarcode': 'bcr_sample_barcode', 'AliquotBarcode': 'RNASeq_Sample_Barcode'})
# groupby sample
idx = tcga_tcr['bcr_sample_barcode'].str.match(r'^TCGA') & (tcga_tcr['chain'] == 'beta')

##some cases have more than one RNA sample, so have to combine them into lists
tcr_counts = tcga_tcr.loc[idx].groupby(['bcr_sample_barcode', 'RNASeq_Sample_Barcode'])['abundance'].sum().to_frame().reset_index().rename(columns={'abundance': 'tcr_counts'})
tcr_counts.unstack(level=-1)
tcr_samples = tcr_counts.groupby('bcr_sample_barcode')['RNASeq_Sample_Barcode'].apply(list)
tcr_samples_counts = tcr_counts.groupby('bcr_sample_barcode')['tcr_counts'].apply(list)
tcr_counts = pd.merge(tcr_samples, tcr_samples_counts, left_index=True, right_index=True)

# merge to sample table by sample id
tcga_sample_table = pd.merge(tcga_sample_table, tcr_counts, how='left', on='bcr_sample_barcode')
...
with open(read_path + 'responses.pkl', 'rb') as f:
                        ...
                        if cancer_dict[case] == 'LAML' and 'TCGA' not in k['experiment_name']:
                            ...
                            if cancer_dict[case] == 'LUSC':
                                ...
                                if case in ['TCGA-HR-A2OH', 'TCGA-HR-A2OG', 'TCGA-D9-A4Z6', 'TCGA-D9-A1X3']:
                                    ...
                                    ##handle the mislabeled BRCA samples here
                                    if case in ['TCGA-BH-A1EU','TCGA-A2-A04T','TCGA-E2-A15I']:
                                        if k['experiment_name'] == 'H_LS-A8-A07R-01A-21W-A050-09-1':
                                            key=case_to_barcode['TCGA-A2-A04T']
                                            Y = True
                                        elif k['experiment_name'] == 'H_LS-C8-A1HM-10A-01D-A135-09':
                                            key = case_to_barcode['TCGA-E2-A15I']
                                            Y = True
                                        elif k['experiment_name'] == 'H_LS-E2-A15I-10A-01D-A135-09':
                                            key = case_to_barcode['TCGA-BH-A1EU']
                                            Y = True
                                        else:
                                            pass
                    if Y== False:
...
pickle.dump(tcga_sample_table, open(write_path + 'tcga_sample_table.pkl', 'wb'))


path_to_genome = read_path + 'chromosomes/'
chromosomes = {}
...
gff = pd.read_csv(read_path + 'Homo_sapiens.GRCh37.87.gff3',
                  sep='\t',
...
genie = pd.read_csv('/home/janaya2/Desktop/ATGC_paper/figures/tmb/genie/files/genomic_information_8.3-consortium.txt', sep='\t', low_memory=False)
...
agilent_df = pd.read_csv(read_path + 'whole_exome_agilent_1.1_refseq_plus_3_boosters.targetIntervals.bed', sep='\t', low_memory=False, header=None)
...
pickle.dump(tcga_maf, open(write_path + 'tcga_maf_table.pkl', 'wb'))
pickle.dump(panel_df, open(write_path + 'tcga_panel_table.pkl', 'wb'))
